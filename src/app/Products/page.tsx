...
import ModernProductCard from "@/components/ProductCard"; // Adjust path if needed
...
interface StarProps {
  key: string;
  style: { width: number; height: number; left: string; top: string; };
  animate: { x: number; y: number; opacity: number[]; scale: number[]; };
  transition: { duration: number; repeat: number; repeatType: RepeatType; delay: number; };
}
...
const ProductsPage = () => {
  ...
  useEffect(() => {
    ...
    const generatedStars: StarProps[] = [...Array(30)].map((_, i) => ({
      ...
      animate: { x: (Math.random() - 0.5) * 50, y: (Math.random() - 0.5) * 50, opacity: [0, 1, 0.5, 0], scale: [1, 1.2, 1] },
      transition: { duration: Math.random() * 10 + 10, repeat: Infinity, repeatType: "mirror", delay: Math.random() * 5 },
    }));
  ...
  const handleAddToCart = (item: Item) => {
    ...
    // Here you would add logic to update your global cart state
  };

  const pageContainerVariants = { initial: { opacity: 0 }, animate: { opacity: 1, transition: { duration: 0.5, staggerChildren: 0.15 } } } as const;
  const heroTextVariants = { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "circOut" } } } as const;
  const features = [
    { icon: Flame, text: "Small-Batch Roasted", delay: 0.8 }, { icon: Leaf, text: "All-Natural Spices", delay: 0.95 },
    { icon: Sparkles, text: "Seriously Bold Flavors", delay: 1.1 }, { icon: Package, text: "Freshness Sealed", delay: 1.25 },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <motion.div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(circle at 30% 70%, #842d0bAA 0%, #11182700 30%), radial-gradient(circle at 70% 30%, #d97706AA 0%, #11182700 25%)", backgroundRepeat: "no-repeat" }} animate={{ backgroundPosition: ["0% 0%", "100% 100%", "0% 0%"] }} transition={{ duration: 40, ease: "linear", repeat: Infinity, repeatType: "mirror" }} />
        {stars.map((star) => <motion.div key={star.key} className="absolute rounded-full bg-amber-400/30" style={star.style} animate={star.animate} transition={star.transition} />)}
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
        <div className="text-center mb-20 md:mb-28">
          ...
          <motion.div className="flex flex-wrap justify-center gap-x-6 gap-y-4 md:gap-x-10 mb-12">
            {features.map((feature) => (
              <motion.div key={feature.text} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: feature.delay, ease:"easeOut" }} className="flex items-center bg-gray-800/60 backdrop-blur-sm text-slate-200 py-2.5 px-5 rounded-full shadow-lg border border-gray-700/50">
        ...
        <motion.div variants={heroTextVariants} className="mb-12 text-center md:text-left">
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            <motion.div key="empty" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center py-20 text-slate-400">
            ...
            <motion.div key="itemsGrid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-10 md:gap-x-8 md:gap-y-12" variants={{ animate: { transition: { staggerChildren: 0.1 } } }}>
