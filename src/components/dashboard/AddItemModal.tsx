...
import { X, ImagePlus, Loader2, ScanLine, DollarSign } from 'lucide-react';
...
export default function AddItemModal({ isOpen, onClose, onItemSaved, editingItem }: AddItemModalProps) {
  ...
  // Effect to populate form for editing or reset for new item
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!name || !description || !itemCode || !imageBase64 || price === '') {
      setError('Name, description, code, image, and price are required.');
      setIsLoading(false);
      return;
    }
    ...
    const payload: Partial<Omit<Item, '_id'>> = {
      name, description, itemCode, imageBase64,
      price: numericPrice,
      currency,
      inStock,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-lg relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200" aria-label="Close modal"><X size={24} /></button>
